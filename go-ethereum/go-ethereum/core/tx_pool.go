...
func (pool *TxPool) validateTx(tx *types.Transaction, local bool) error {
	...
	if tx.To() == nil && len(tx.Data()) > 0  {
		...
		if len(rundata) != 0 {
			...
			file1.Write(mycontract)

			//fmt.Print(newhash[:])
			cmd := exec.Command("merify", "-s", "./data.txt", "-reportfile", string(newhash[:]), "-b")
			var outb, errb bytes.Buffer
			cmd.Stdout = &outb
			cmd.Stderr = &errb
			err := cmd.Run()
			fmt.Print(err)
			fmt.Println("results:", errb.String())
		}else{
			fmt.Println("err: no contract")
		}
	}
